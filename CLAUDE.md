...
**Dữ liệu tồn kho thật của bệnh viện**: `docs/Danh mục TBYT.xlsx` (chưa track trong Git — file người dùng để lại trong repo cục bộ, không phải do Claude tạo) chứa danh mục ~217 thiết bị thật đang có tại BVDK Đông Sơn, đối chiếu để biết hệ thống cần hiển thị trường nào. Các cột khớp thẳng vào `01_THIET_BI` đã có: MODEL, SERIAL, NAM_SX, HANG_SX, NUOC_SX, KHOA_PHONG, TG_SD→NGAY_DUA_VAO_SU_DUNG, PL_TBYT→PHAN_LOAI, HINH_THUC_MS→HINH_THUC_MUA_SAM, Tình trạng→TINH_TRANG_KY_THUAT, các cột "...lần cuối/kế tiếp" (Kiểm định/ATBX/Bảo trì) khớp đúng các sheet nghiệp vụ tương ứng (15/17/14). Các cột **chưa có chỗ trong schema hiện tại** (chưa tự ý thêm cột — chờ xác nhận nếu cần): "Mã khai BH" (mã khai bảo hiểm), "SỐ GIAY_PHEP NK /SỐ GPLH" (số giấy phép nhập khẩu/lưu hành — dữ liệu pháp lý quan trọng), "NGUON_TS" (nguồn kinh phí: TPCP/TTĐ/Đơn vị/Dự án...), "NGUYEN_GIA"/"DOC_BANG_CHU" (nguyên giá), "SO_LUONG"/"DON_VI". Cột "Chi tiết 1-3"/"Model - Seria 1-3" là mô tả phụ kiện/linh kiện đi kèm dạng text tự do, rất dài — không phải dữ liệu 32_PHU_TUNG (đó là phụ tùng thay khi sửa chữa, khác bản chất).
